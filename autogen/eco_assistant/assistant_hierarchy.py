...
class SubAssistant(AssistantAgent):
    def __init__(self, name, model, system_message=DEFAULT_SYSTEM_MESSAGE, **config):
        ...
        self.model = model
        self.oai_wrapper = OpenAIWrapper()
    ...
    def generate_oai_reply(self, messages: List[Dict]):
        ...
        try:
            response = self.oai_wrapper.create( context=context, messages=messages, model=self.model)
            ...
            extracted_response = self.oai_wrapper.extract_text_or_function_call(response)[0]
        ...
        print("Response from OAI wrapper:", self.model, "*"*20)
        print(colored(extracted_response, "green"), flush=True)
        return extracted_response, prompt_tokens, completion_tokens
