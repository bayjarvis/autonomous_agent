...
class RetrievalAgent(UserProxyAgent):
    ...
    def __init__(
        ...
        file = os.path.join(".", f'{name}_retrieval_log.json')
        if os.path.exists(file):
            with open(file, 'r') as f:
                self.retrieval_log = json.load(f)
        else:
            self.retrieval_log = {}
        file = os.path.join(".", f'{name}_query_to_code.json')
        if os.path.exists(file):
            with open(file, 'r') as f:
                self.query_to_code = json.load(f)
        else:
            self.query_to_code = {}
    ...
    def run_code(self, code, lang="python", **kwargs):
        ...
        result = execute_code(code, lang=lang, **kwargs)
        print("Result:", "*"*50)
