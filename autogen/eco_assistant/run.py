...
from assistant_hierarchy import initial_assistant_hierarchy
...
def main(args, queries):
    ...
    with open(os.path.join(".", f'{retrieval_agent_name}_retrieval_log.json'), 'w') as f:
        json.dump(user.retrieval_log, f)
    with open(os.path.join(".", f'{retrieval_agent_name}_query_to_code.json'), 'w') as f:
        json.dump(user.query_to_code, f)
    stop()
    return logs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default="gpt-3.5-turbo-16k,gpt-4-1106-preview")
